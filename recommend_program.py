...
def generation_oneSector(number_, cor_dict_):
    orig_cor_list_=[]
...
def generation_manySectors(number_list_, cor_dict_list_) : # caution : 두 parameter 의 len()은 같아야함
    orig_cor_list_ = []
...
NUM_FOLDER = 10 # 전체 폴더 수
FIXVALUE = 0.008  # 1km는 약 0.008도 이다

#### 난수 발생 dictonary 선택
## 1. generation_manySectors method를 사용
# 관악구_서울
...
cor_number_list = [300,150,150] # Σvalue = 노인 인구수
...
# cor_number_list=[300,150,300,150,250]
# recommend_number_list = [4,2,4,2,3]

## 2. generation_oneSector method를 사용
...
# 시작 노드
orig_cor_list = generation_manySectors(cor_number_list, cor_dict_list)
...
for i in range(NUM_FOLDER) :
    # 추천 노드 
    recommend_cor_list = generation_manySectors(recommend_number_list, cor_dict_list)
